...
function SidebarCards({
  ...
  restoreQuote, handleGlobalDiscountChange, setDeleteConfirm, onEditValidUntil,
}: SidebarCardsProps) {
    ...
    try {
      ...
      saveInvoice({
        ...
        items: payload.items.map((item) => {
          ...
          return { name: item.name, unit: item.unit, qty: item.qty, unitPriceNet: item.unitPriceNet, vatRate: item.vatRate, comment: item.comment, net, vat, gross };
        }),
      ...
      >
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
            <Input label={invoiceForm.buyerType === 'company' ? t.invoices.buyerCompanyName : t.invoices.buyerName} value={invoiceForm.buyerName} onChange={(e) => setInvoiceForm((prev) => ({ ...prev, buyerName: e.target.value }))} />
          ...
          {invoiceForm.buyerType === 'individual' ? (
            ...
            <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
              <Input label={t.invoices.buyerTaxNumber} value={invoiceForm.buyerTaxNumber} onChange={(e) => setInvoiceForm((prev) => ({ ...prev, buyerTaxNumber: e.target.value }))} />
              <Input label={t.invoices.buyerCompanyAddress} value={invoiceForm.buyerCompanyAddress} onChange={(e) => setInvoiceForm((prev) => ({ ...prev, buyerCompanyAddress: e.target.value }))} />
            </div>
          )}
          ...
          <TextArea label={t.invoices.comment} value={invoiceComment} onChange={(e) => setInvoiceComment(e.target.value)} rows={3} />
